...
BLENDER_INSTALLATION_PATH = '/jumbo/yuwingtai/junwei/blender/'
...
def _render(file_path, sha256, output_dir, num_views):
    ...
    # # 允许的 pitch 范围，40度 → 弧度
    # pitch_max = np.deg2rad(40)
    # pitch_min = np.deg2rad(-40)

    # # 过滤 views
    # filtered_views = [v for v in views if pitch_min <= v['pitch'] <= pitch_max]

    # print(f"原始相机数量: {len(views)}, 过滤后数量: {len(filtered_views)}")
    # ================= standard =================== #
    r = [2] * 8
    fov = [40 / 180 * np.pi] * 8
    yaws1 = [0/180*np.pi, 45/180*np.pi, 90/180*np.pi, 135/180*np.pi, 180/180*np.pi, 225/180*np.pi, 270/180*np.pi, 315/180*np.pi]
    yaws = [yaws1[2], yaws1[1], yaws1[0], yaws1[7], yaws1[6], yaws1[5], yaws1[4], yaws1[3],]
    pitchs = [30/180*np.pi for _ in range(8)]
    views = [{'yaw': y, 'pitch': p, 'radius': r, 'fov': f} for y, p, r, f in zip(yaws, pitchs, r, fov)]
    filtered_views = [v for v in views]
    
    args = [
        BLENDER_PATH, '-b', '-P', os.path.join(os.path.dirname(__file__), 'blender_script', 'render.py'),
        '--',
        '--views', json.dumps(filtered_views),
