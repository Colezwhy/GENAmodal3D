...
class ModulatedSparseTransformerCrossBlockMaskAsCondWeighted(nn.Module):
    ...
    def __init__(
        ...
        qkv_bias: bool = True,
        share_mod: bool = False,
        if_view_wise: bool = False
    ):
        super().__init__()
        self.view_wise = if_view_wise
        self.use_checkpoint = use_checkpoint
        self.share_mod = share_mod
        self.norm1 = LayerNorm32(channels, elementwise_affine=False, eps=1e-6)
        self.norm2 = LayerNorm32(channels, elementwise_affine=True, eps=1e-6)
    ...
    def _forward(self, x: SparseTensor, mod: torch.Tensor, context: dict) -> SparseTensor:
        ...
        if self.view_wise:
            # print('using view-wise texture..')
            dim = 37 * 37
            # =========== view_wise =========== #
            contexts = torch.split(context['mask'], dim, dim=1)
            dim_img = 1374
            contexts_img = torch.split(context['cond'], dim_img, dim=1)
            contexts_occ = torch.split(context['mask_occ'], dim, dim=1)
            outputs = []
            outputs_intermediate = []
            for i, c in enumerate(contexts):
                h_ = self.cross_attn(h, contexts_img[i], mask_weight = c)
                x_ = x + h_
                outputs_intermediate.append(x_.feats)
                h_ = x_.replace(self.norm_mask(x_.feats))
                h_ = self.cross_attn_mask(h_, contexts_occ[i])
                outputs.append(h_.feats)
            x = x.replace(torch.stack(outputs_intermediate, dim=0).mean(dim=0))
            h = h.replace(torch.stack(outputs, dim=0).mean(dim=0))
            x = x + h
            h = x.replace(self.norm3(x.feats))
            h = h * (1 + scale_mlp) + shift_mlp
            h = self.mlp(h)
            ...
            # print(context['cond'].shape, context['mask'].shape, context['mask_occ'].shape)
